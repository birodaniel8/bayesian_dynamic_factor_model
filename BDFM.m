classdef BDFM
    ...
    methods
        ...
        function [sampled_beta, sampled_sigma_squared, sampled_factor] = estimate_normal_model(obj, ndraw, burnin, display)
            if nargin == 3, display = true; end
            % create containers:
            sampled_beta = zeros(obj.m, obj.k, ndraw-burnin);
            sampled_sigma_squared = zeros(obj.m, ndraw-burnin);
            sampled_factor = zeros(obj.T, obj.k, ndraw-burnin);
            % initial values:
